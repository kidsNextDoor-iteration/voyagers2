...
const TripDetails = ({ tripId, closePopup, fetchTrips }) => {
  ...
  const submitTraveler = () => {
    ...
    fetch('/trip/addTraveler', {
      ...
      .then(res => res.json())
      .then(data => {
        if (data === 'Traveler Added') {
          setSubmissionResponse('Traveler Added')
          setTravelerEmail('')

        } else {
          setSubmissionResponse('Invite Email Sent!')
        }
        ...
        <div className="trip-content">
          ...
          <div className='trip-content-right-container'>
            <div className='right-top-text-container'>
              ...
              <div id="response">{submissionResponse}</div>
              {/* <hr className='trip-details-hr' /> */}
            </div>
